...
struct GetPlansDetailDataService
{
    static let shared = GetPlansDetailDataService()
    func getPlansDetail(postID: String, completion : @escaping (NetworkResult<Any>) -> Void)
    {
...
//            dump(dataResponse)
    ...
    private func isValidData(data : Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(PlansDetailDataModel.self, from: data)
        else {return .pathErr}
